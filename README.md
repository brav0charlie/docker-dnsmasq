...
Based on Alpine Linux 'latest' (currently 3.16.x) and dnsmasq-2.86-r3.

💡 To build a version of docker-dnsmasq using the latest pre-release version of Alpine Linux Edge and dnsmasq-2.86-r5, use `Dockerfile-edge` in the `docker-dnsmasq` directory. The easiest way to do this is to `mv Dockerfile Dockerfile-release` and then `mv Dockerfile-edge Dockerfile` and then build the container. Be sure to also change the version in the `image:` name in `docker-compose.yml` so your image gets built with the correct version.
