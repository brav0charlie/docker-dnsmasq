...
💡 To build a version of docker-dnsmasq using the latest pre-release version of Alpine Linux Edge and dnsmasq-2.89-r5, use `Dockerfile-edge`. The easiest way to do this is to `mv Dockerfile Dockerfile-release` and then `mv Dockerfile-edge Dockerfile` and then build the container. Be sure to also change the version in the `image:` name in `docker-compose.yml` so your image gets built with the correct version.
